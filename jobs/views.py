...
class JobPDFView(APIView):
    ...
    def get(self, request, pk):
        ...
        filename = f'CanadianMdJobs_{safe_title}_{job.id}.pdf'
    ...
    def _build_pdf(self, job):
        ...
        header_table = Table(
            [[Paragraph('CanadianMdJobs', header_style), Paragraph(f'Generated: {gen_date}', date_style)]],
            colWidths=['70%', '30%'],
        )
        ...
        _add_section('How to Apply', getattr(job, 'how_to_apply', '') or (
            f'Please apply through CanadianMdJobs. '
            f'Contact {employer_name} for more information.' if employer_name else
            'Please apply through CanadianMdJobs.'
        ))
        ...
        story.append(Paragraph(
            'This document was generated by CanadianMdJobs — connecting physicians with healthcare employers across Canada. '
            'Visit us at candianmdjobs.ca for more opportunities.',
            footer_style,
        ))
